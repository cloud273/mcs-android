...
import com.cloud273.doctor.model.isToday
...
import kotlinx.android.synthetic.main.fragment_md_list_appointment.*
import java.util.*

class MDListAppointmentFragment: McsFragment(), RecyclerCellInterface {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        return inflater.inflate(R.layout.fragment_md_list_appointment, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        segmentView.leftText = "Today".localized
        segmentView.rightText = "All".localized
        segmentView.sideChangedListener = { isLeft ->
            isAll = !isLeft
        }
        refresh()
    }
    ...
    private fun loadAppointments(completion: (Boolean) -> Unit) {
        ...
        if (token != null) {
            val to = Date().add(month = 1)
            val from = Date().add(second = McsAppConfiguration.instance.beginableFrom)
            McsDoctorListAppointmentApi(token, packageType, arrayListOf(McsAptStatusType.accepted, McsAptStatusType.started), from, to).run { success, appointments, _ ->
                if (success) {
    ...
    private fun noAppointmentText(): String {
        return if (isAll) "No_appointment_doctor_message".localized else "No_appointment_today_doctor_message".localized
    }

    private fun reloadView() {
        val cells = mutableListOf<RecyclerCellData>()
        for (apt in appointments) {
            if (isAll || apt.isToday) cells.add(RecyclerCellData(id = "appointment", data = apt, resource = MDAppointmentCell.cellId))
        }
        if (cells.isEmpty()) {
            ...
            McsDoctorAppointmentDetailApi(MDDatabase.instance.token!!, apt.id!!).run { success, appointment, code ->
                if (success) {
                    if (appointment!!.finishable!!) {
                        val bundle = Bundle()
                        bundle.putString(MDPrescriptionFragment.appointmentKey, SupportCenter.instance.push(appointment))
                        navigate(R.id.mdPrescriptionFragment, bundle)
                    } else {
                        val bundle = Bundle()
                        bundle.putString(MDAppointmentFragment.appointmentKey, SupportCenter.instance.push(appointment))
                        navigate(R.id.mdAppointmentFragment, bundle)
                    }
                } else {
                    if (code == 404) {
                        refresh()
