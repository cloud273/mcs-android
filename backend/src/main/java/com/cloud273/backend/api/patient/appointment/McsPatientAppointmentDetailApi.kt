...
class McsPatientAppointmentDetailApi (token: String, private var id: Int) : McsUserRequestApi(token) {
    ...
    fun run(completion: (success: Boolean, appointment: McsAppointment?, code: Int) -> Unit) {
        commonFetch(Dispatchers.Main) { code, _, body ->
            if (code == 200) {
                val data = body?.toObject<McsAppointment>(McsAppointment::class.java)
                if (data != null) {
