...
class McsDoctorListAppointmentApi(token: String, private val type: McsPackageType, private val statuses: List<McsAptStatusType>? = null, private val from: Date, private val to: Date): McsUserRequestApi(token) {
    ...
    fun run(completion: (success: Boolean, appointments: List<McsAppointment>?, code: Int) -> Unit) {
        commonFetch(Dispatchers.Main) { code, _, body ->
            if (code == 200) {
                val data = body?.toArray<McsAppointment>(
